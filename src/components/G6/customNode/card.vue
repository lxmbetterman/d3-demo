<!--  -->
<template>
  <div id="CustomNodeCard">/customNode/card</div>
</template>
...
export default {
  ...
  methods: {
    init() {
        ...
        createNodeMarker: (group, collapsed, x, y) => {
          ...
          group.addShape('marker', {
            attrs: {
              ...
              y,
              r: 7,
              symbol: collapsed ? EXPAND_ICON : EXPAND_ICON,
        ...
        draw: (cfg, group) => {
          ...
          if (cfg.ip) {
            ...
            const ipRect = group.addShape('rect', {
              attrs: {
                fill: nodeError ? 'blue' : '#FFF',
                stroke: nodeError ? 'red' : null,
                radius: 2,
                cursor: 'pointer'
              },
            ...
            /* ipBox */
            ipRect.attr({ // 文字的rect是根据文字动态计算 x/y/width/heigh的
      ...
      const container = document.getElementById('CustomNodeCard')
      const width = container.scrollWidth
      const height = 500
      graph = new G6.Graph({
        container: 'CustomNodeCard',
        width,
        height,
